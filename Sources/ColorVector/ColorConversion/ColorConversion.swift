...
public enum ColorConversion {
  ...
  struct SpaceTuple: Codable, Equatable, Hashable, Sendable {
    /// source color space
...
extension ColorConversion {
  private typealias ConversionFunction = @Sendable (ColorVector) -> ColorVector
  ...
  private static let functionMap: [SpaceTuple: ConversionFunctionChain] = {
    ...
    func dfs_search(
      _ from: ColorVector.Space, _ space: ColorVector.Space, _ chain: ConversionFunctionChain
    ) {
  ...
  fileprivate static func convert_from_self_to_self(_ vector: ColorVector) -> ColorVector {
    vector
  }
}
